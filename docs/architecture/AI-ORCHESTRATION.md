...
用户消息先在 Rust 边界做 UTF-8、空白、最大 4,000 字符与危险控制字符校验，然后持久化。LLM 请求失败时保留用户消息，并显示固定的本地可用性提示“现在无法使用 AI 对话；本地电台、播放控制和反馈仍然可用。”；该提示以 `provider=local`、`model=deterministic` 保存以便恢复界面，明确不是模型生成的 AI 回复，不创建 Memory Proposal，也不自动重放可能计费的请求。输出只作为文字；任何播放意图必须来自显式 UI command，而不是从自由文本模型输出直接执行。
