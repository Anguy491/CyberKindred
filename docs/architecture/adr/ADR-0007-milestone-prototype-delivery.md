...
- Product Owner 以 milestone 为单位验收。任务是内部计划与交付切片；`Done` 只表示已集成并完成适当自检，不表示逐任务获用户接受。
- 同一时刻只有一个 Active milestone。milestone 内依赖满足、文件写入不重叠的任务可受控并行；下一 milestone 默认等待当前 checkpoint 被接受。
...
- checkpoint 必须如实列出 candidate commit、环境、完成范围、执行结果、失败与未执行检查、影响、规避方式和目标 milestone。Product Owner 可以带非关键债务接受，但失败或未执行不能改写成通过。
...
- 用户看到的验收次数从每个 task 一次降为每个 milestone 一次，开发可在 milestone 内连续推进和并行处理独立切片。
