...
`LibraryRoot` exposes `{ rootId, displayName, available }`; absolute paths never cross into WebView. `TrackFilters` is `{ availability: "playable" | "missing" | null; matchStatus: "matched" | "unmatched" | "review" | null }`；两个字段 AND 组合，null 表示不过滤。`TrackView` is the exact DTO `{ trackId: string; availability: "playable" | "missing" | "corrupt" | "unsupported"; durationMs: number; artworkAvailable: boolean; original: { title: string | null; artist: string | null; album: string | null }; enriched: { title: string | null; artist: string | null; album: string | null; provider: "musicbrainz"; confidence: number; fetchedAt: string } | null; matchStatus: "matched" | "unmatched" | "review" }`, and never includes an absolute or relative path。`track_external_metadata.match_status` maps `adopted`→`matched`, `suggested`→`review`, and absent/`no_match`→`unmatched`; an adopted or suggested cached result supplies `enriched` and its authoritative `fetchedAt`, while `no_match` supplies `enriched: null`。搜索同时匹配 original/enriched title、artist、album 且保留 filters。Empty `rootIds` in API-013 means all configured roots. Scan progress is delivered by EVT-005.
