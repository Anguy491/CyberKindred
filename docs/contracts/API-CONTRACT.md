...
| API-002 `api_v1_get_onboarding_state` | `EmptyRequest` | `{ completed: boolean; completedSteps: string[]; revision: number }` | 2 s | Read, idempotent |
| API-003 `api_v1_save_onboarding_profile` | `{ clientRequestId; expectedRevision; profile: OnboardingProfile }` | `Ack` | 5 s | 10-minute key |
...
`OnboardingProfile` is `{ displayName: string; companionStyle: "quiet_warm"; initialPreferences: string[]; narrationDensity: "quiet" | "balanced" | "frequent"; privacyAccepted: true }`。`displayName` 为 0..80 字符（空字符串表示不使用称呼），`initialPreferences` 为 0..20 项且每项 1..100 字符。该 DTO 不含城市、坐标、`WeatherLocation` 或 schedule；引导中的城市步骤必须先调用 API-048 再由用户选择并调用 API-049，可选日程必须调用 API-033。API-003 只保存画像字段与隐私确认状态，不能成为位置或日程的第二条 revision/validation 写入路径。
