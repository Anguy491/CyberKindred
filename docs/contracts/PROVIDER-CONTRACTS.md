...
Credential validation 与 model capability probe 是两个不同动作。API-004 使用候选 origin/key 对固定 `/v1/models` 执行最小只读验证；它只能证明 credential 被该 origin 接受，不声明某个 model 支持 Structured Outputs。Model capability probe 使用候选 `providerOrigin`、候选 `llmModelId` 与该 origin 已验证 credential，向固定 `/v1/responses` 发送最小请求：`store:false`、无 tools、`reasoning.effort=none`，并要求 strict JSON Schema `{"ready":true}`。API-008 只在 `llmModelId` 实际变化时、保存任何 patch 字段前执行 model capability probe，deadline 为 60 秒；失败时整个 patch 不持久化。不含已变化 `llmModelId` 的 settings patch 不调用 provider。API-006 使用同一 model capability probe 规则测试当前已保存配置。
