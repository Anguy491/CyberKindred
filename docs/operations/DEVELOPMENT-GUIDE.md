...
4. When all planned slices are integrated, build one candidate and run the milestone checkpoint subset from `TEST-STRATEGY.md`. For M1–M6, record `Passed` or `Passed with known gaps` and immediately continue into the next milestone; stop only on `Blocked` or an authorization boundary.
