...
| RISK-015 | WebdriverIO 9.31.5 的传递开发依赖存在未缓解的 High advisory | M | H | 完整 `pnpm audit --audit-level high` 在 `extract-zip 2.0.1`、`deepmerge-ts 7.1.6` 与 `serialize-javascript 6.0.2` 报警；其中审计建议的 `extract-zip >=2.0.2` 在 registry 尚不可安装 | 已在 `2ae920e` 移除该 npm 链，改由 repository-owned Node 24 W3C harness 直接驱动固定 `tauri-driver`；lockfile/frozen install、完整 audit、9 条 self-test 和匹配 EdgeDriver 的隔离 desktop smoke 均通过，Playwright 未冒充 desktop E2E | Lead/QA | Closed |
| RISK-016 | authoritative credential 生命周期语义冲突 | M | H | origin 切换实现误删仍需使用、但非当前选中的 credential，或 API-005 删除范围扩大 | 2026-09-03 已决定保留各 canonical origin credential，直到 API-005 精确删除或 full reset 全量删除；权威文档已同步，完成 API-004/API-005 实现与 canary 复验后关闭 | Product/Security | Mitigating |
| RISK-017 | operation terminal event 无法在当前 Tauri bus/outbox 协议下证明 delivery exactly-once | M | H | emit 成功而 delivered 标记前崩溃会跨进程重放；先标记再 emit 则可能永久丢失 | 2026-09-03 已决定权威 terminal exactly-one、transport at-least-once、前端按 `operationId` 幂等；公共契约已同步，完成 duplicate/replay/conflicting-terminal 测试和安全复核后关闭 | Core/Product | Mitigating |
| RISK-018 | model ID 保存前 capability probe 与 API-006/API-008 契约流程不相容 | M | M | model patch 未 probe 即持久化，或 probe 失败后出现部分设置写入 | 2026-09-03 已决定 API-008 在 `llmModelId` 实际变化时以候选配置执行 60 秒 pre-save probe；成功后原子保存整个 patch，失败零设置变化，完成实现/timeout/no-call 测试后关闭 | AI/Product | Mitigating |
...
- The M2 blocked checkpoint is recorded in [`M2.md`](../testing/checkpoints/M2.md). `RISK-015` is closed: the WebdriverIO tree was removed, the full npm audit is clean, and an isolated Windows desktop shell smoke passed with matching signed EdgeDriver. Product Owner resolved the three public semantics on 2026-09-03; `RISK-016`–`RISK-018` remain `Mitigating`, and M2 remains active until implementation and hard-gate revalidation close them.
- The provider worktree persists API-009 acceptance before spawning, atomically converts accepted operations to one authoritative terminal, recovers bounded batches without a 100-row tail, expires delivered/undelivered outbox records at 24-hour/7-day boundaries, and serializes credential mutations. The approved contract now truthfully declares at-least-once transport and requires frontend `operationId` dedupe.
- The model-setting remediation uses the existing fixed hermetic Responses probe (`store:false`, no tools, strict JSON Schema, `reasoning.effort=none`) before saving an actually changed model ID, with a 60-second API-008 path and atomic patch failure. No real provider request is authorized or required for the default suite.
