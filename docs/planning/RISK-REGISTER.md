...
| RISK-015 | WebdriverIO 9.31.5 的传递开发依赖存在未缓解的 High advisory | M | H | 完整 `pnpm audit --audit-level high` 在 `extract-zip 2.0.1`、`deepmerge-ts 7.1.6` 与 `serialize-javascript 6.0.2` 报警；其中审计建议的 `extract-zip >=2.0.2` 在 registry 尚不可安装 | M2 已批准移除该 npm 链，改由 repository-owned Node 24 W3C harness 直接驱动固定的 `tauri-driver`；关闭仍须提供 lockfile 差异、完整 npm audit、harness 自检及 desktop smoke 证据，Playwright 不冒充 desktop E2E | Lead/QA | Mitigating |
