...
- The M2 blocked checkpoint is recorded in [`M2.md`](../testing/checkpoints/M2.md). `RISK-015` is closed: the WebdriverIO tree was removed, the full npm audit is clean, and an isolated Windows desktop shell smoke passed with matching signed EdgeDriver. Product Owner resolved the three public semantics on 2026-09-03; `TASK-008` hermetic suites and independent read-only review closed `RISK-016`–`RISK-018`. M2 remains active for `TASK-010` and the final checkpoint rerun.
