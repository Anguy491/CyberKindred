...
`UX-RAD-003`：播放控件顺序固定为上一首、播放/暂停、下一首、进度；下方是喜欢、跳过、少说一点。来源不支持的操作保留布局但呈 disabled，并在聚焦/悬停时显示内联原因，例如 `[UNAVAILABLE: APPLE MUSIC SESSION]`，不能静默失败。未连接的 Apple `system_session` 来源选择按钮保持可操作，因为该点击本身是发起显式连接的用户意图；连接前不得枚举或读取其会话。
...
`UX-SET-003`：Apple Music 组先显示内存中的当前 Windows App 会话状态和能力清单，再显示连接指引与“连接 / 重新检查会话”操作。打开该组本身不得枚举或读取 GSMTC；只有用户触发连接操作后才发现并绑定 Apple Music Windows App 会话。无 App/无会话/会话受限分别使用不同文字；绝不提供“登录 MusicKit”或“控制网页”入口。
