...
当前仓库已完成 M1，并处于 M2 blocked checkpoint 的 remediation。下表控制混合了已由 hermetic 测试/独立只读复核证明的 M2 基础实现与尚待后续 milestone 的设计要求；每一行明确区分 evidence 和 unknown。WebdriverIO 依赖 High 已通过移除依赖链和隔离 desktop smoke 关闭；Product Owner 已在 2026-09-03 选择 credential 生命周期、terminal delivery 与 model pre-save probe 语义，M2 在对应实现和 hard-gate 复验完成前仍不转场，详见 [`M2 checkpoint`](../testing/checkpoints/M2.md)。
