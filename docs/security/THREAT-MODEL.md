...
|---|---|---|---|---|---|---|
| Per-user desktop | OpenAI-compatible API Key | User-confirmed origin; credential namespaced by canonical origin | Credential target under `CyberKindred/provider/…`; literal never persisted elsewhere | Rust provider client only; selected HTTPS origin receives Authorization | Credential Manager ACL, no IPC getter, per-origin delete；reset enumerates all CyberKindred origins；log/export canaries | M1 standard-user live canary and M2 internal wrapper/fake canaries pass；origin-switch retention remains blocked by `RISK-016` |
...
| Reliable local delivery | Operation outbox | Rust repository only；no generic payload serialization | SQLite rows containing kind/opaque IDs/sequence/attempt/status/time/count only | Rust dispatcher；WebView receives allowlisted event DTO | typed per-event projection, no body/path/secret/GSMTC metadata, 24 h/7 d expiry | M2 accepted→terminal/recovery/retention/canary tests pass；transport exactly-once remains blocked by `RISK-017` |
