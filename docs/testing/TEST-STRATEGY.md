...
| Desktop E2E | Repository-owned Node 24 W3C WebDriver harness + `tauri-driver` on Windows | onboarding、navigation、radio、settings、restart | Current M2 shell makes no provider call; later provider cases require injected fakes |
...
`pnpm test:e2e` uses only Node 24 built-ins to speak the W3C WebDriver protocol to the pinned `tauri-driver`; it does not depend on WebdriverIO or download a browser driver during `pnpm install`. The harness builds the unpackaged desktop executable with `src-tauri/tauri.e2e.conf.json`, whose dedicated `com.cyberkindred.desktop.e2e` identifier prevents it from opening production app-data roots. It owns a loopback-only driver process/session, always attempts session deletion and process cleanup, and accepts an optional case-name filter after `--`. The M2 shell cases invoke no provider, credential mutation, picker, notification or audio action; a later scenario that exercises those boundaries must first add injected E2E fakes and may not use production credentials or network. Windows CI must provision the Edge WebDriver that matches the installed WebView2/Edge runtime before invoking this command. Renderer-only Playwright evidence remains a separate layer and never substitutes for this desktop path.
