...
M2 的当前实现证据与 hard-gate 失败由 [`M2 checkpoint`](checkpoints/M2.md) 及其 [`evidence manifest`](../../artifacts/test-evidence/milestones/M2/manifest.json) 聚合记录。`TASK-004`–`TASK-007` 与 `TASK-009` 已集成；Node/W3C desktop shell smoke 为 FR-RAD-004、FR-SET-001、NFR-A11Y-001、NFR-COMPAT-003 提供 M2 candidate 证据。Product Owner 已在 2026-09-03 解决 credential 生命周期、model probe 流程和 terminal delivery 语义，`TASK-008` 已恢复为 `In Progress`；`TASK-010` 与全部 M3 task 在其依赖完成前仍保持 `Blocked`。表中映射不因 partial implementation 改写为通过。
